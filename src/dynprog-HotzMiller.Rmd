...
model.solve <- function(model,p) {
  ...
  Vf = cbind( p$u0 , S*uf(p$r1 * (1:p$ne) + p$rt*p$nt ,p) +p$u1, S*uf(p$r2 * (1:p$ne) + p$rt*p$nt ,p) + p$u2)/p$r
  ...
  for (t in (p$nt-1):1) {
    ...
    Q1 =  S*uf(p$r1 * (1:p$ne) + p$rt*t ,p) + p$u1  + 1/(1+p$r)* GG[2,,] %*% V[t+1,]
    Q2 =  S*uf(p$r2 * (1:p$ne) + p$rt*t ,p) + p$u2  + 1/(1+p$r)* GG[3,,] %*% V[t+1,]
    QQ = cbind(Q0,Q1,Q2)
...
$$V = \mathbb{E} \big[ \max u(a) + \xi(a) \big] = \log \sum_a \exp(u(a)) + \gamma_c$$

we also know that:
$$ Pr[a] = \frac{\exp(u(a))}{\sum_{a'} \exp(u(a'))} $$

but also, we can express $V$ usin these choice probabilities:

$$ V = \sum_a Pr[a] \cdot \big( u(a) + \mathbb{E} [\xi(a)|a] \big) $$
where finally we also know that $\mathbb{E} [\xi(a)|a] = \gamma_c - log(Pr[a])$. This then gives us that:

$$ V = \sum_a Pr[a] \cdot \big( u(a) + \gamma_c - log(Pr[a]) \big) $$

We then use the recursive formulation of the value function which was: 

$$ \bar{V}_t(x) = \log \sum_a \exp \Big[  \mathbb{E} U(w,a,\xi) + \beta \sum_{x_{t+1}}\bar{V}_t(x_{t+1}) g(x_{t+1}|a) \Big] + \gamma_c$$

where we have that $\mathbb{E} U(w,a,\xi) = \gamma S u( \mathbb{E}w)$. At this point we are interested in recovering $u(a)$ and $\gamma$. We can look at the final period and see that

$$ 
\begin{align*}
\bar{V}_T(x) &=  \mathbb{E} \max_a \Big[ \gamma \cdot S \cdot u_0(\mathbb{E}[w|x,t]) + u(a) \Big] \\
&= \sum_a Pr[a|x,t] \Big(  \gamma \cdot S \cdot u_0(\mathbb{E}[w|x,t]) + u(a) + \gamma_c - \log Pr[a|x,t] \Big) \\
&= \gamma \cdot \Big( \sum_a Pr[a|x,t] \cdot S \cdot u_0(\mathbb{E}[w|x,t]) \Big)  + u(a) \cdot \Big( \sum_a Pr[a|x,t] \cdot S \cdot u_0(\mathbb{E}[w|x,t]) \Big)  \\
& \hspace{5cm} + \gamma_c - \sum Pr[a|x,t] \log Pr[a|x,t] \\
 \end{align*}
 $$

Call $\theta = (\gamma,u(2),u(3))$, we have found a matrix $B_T$ which is $n_e \times 4$ and a vector $A_T$, functions only of data and of know parameters, such that $\bar{V}_T = A_T + B_T \theta$. Strictly speaking we could just recover the parameters directly here! But for the sake of the exercice we are going to move backward.

Concretely, at each period $t$ we have that the flow payoff can be expressed linearly. In other words, there exists a $\tilde{B}_t$ such that $\mathbb{E_t}u(w_t) = \tilde{B}_t \theta$. We can now express $A_t$ and $B_t$ recursively:

$$ 
\begin{align*}
\bar{V}_{t} &= \log \sum_a \exp \Big[   S \cdot u_0(\mathbb{E}_{t}w_{t}) + u(a) + \beta \cdot G(a) \times \bar{V}_T \Big] + \gamma_c \\
& = \log \sum_a \exp \Big[   \tilde{B}_{t} \theta + \beta G(a) (A_t +  B_t \theta) \Big] + \gamma_c  \\
& = \sum_a Pr[a|t,x] \Big[    \tilde{B}_{t} \theta + \beta G(a) (A_t +  B_t \theta)  - \log Pr[a|t,x] \Big] + \gamma_c \\
& = \underbrace{\Big( \sum_a Pr[a|t,x] ( \tilde{B}_{t} \theta + \beta G(a) B_t) \Big)}_{B_{t-1}} \theta + \underbrace{ \sum_a Pr[a|t,x] A_{t} + \gamma_c -  \sum_a Pr[a|t,x] \log Pr[a|t,x]}_{A_{t-1}}
\end{align*}
$$

Where we combine the two terms to construct $B_{t-1},A_{t-1}$. The current question asks you to write the code that construct the sequence of $B_{t-1},A_{t-1}$ as a function of measured choice probabilities $Pr[a|x,t]$. Provide such code, and to verify that your code is correct, construct the $B_{t-1},A_{t-1}$ associated with the true probabilities coming constructed from `model$QQa`, then using the true $\theta$, plot your generated $A_t +  B_t \theta$ against the `model$V` which should line on the 45 degree line as in the previous question.

### **Q4** Use the CCP to $\gamma$ and $u(2),u(3)$
