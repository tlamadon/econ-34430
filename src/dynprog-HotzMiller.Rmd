...
$$ U(w,a,\xi) = \frac{w^{1-\rho}-1}{1-\rho} + u(a) +  \xi(a)$$
...
$$ \log(w_{it}(a)) = \beta_a E_{it} + \epsilon_{it}(a) $$
...
$$ \bar{V}_t(x) = \log \sum_a \exp \Big[  E U(w,a,\xi) + \beta \sum_{x_{t+1}}\bar{V}_t(x_{t+1})) g(x_{t+1}|a) \Big]$$
because of the log-normal shocks we get that 
$$E U(w,a,\xi) = \exp \Big( (1-\rho)^2 \sigma_\epsilon^2/2) \Big) \cdot U(w,a,0) .$$
...
p    = list(nt=10, # nb of periods
            ...
            u0=0,u1=2.5,u2=2.25,  # preference values for each choice
            r=Inf+0.02) # discount paramter
...
ggplot(rr[t<10],aes(x=t,y=pr,color=factor(A))) + geom_line() +ggtitle("Average choices over time")

rr = simdata[,list(m=mean(E),q975=quantile(E,0.975),q025=quantile(E,0.025)),t]
ggplot(rr,aes(x=t,y=m,ymin=q025,ymax=q975)) + geom_line() + geom_errorbar(width=0.2) +ggtitle("value of E over time")
```
...
Show that you can recover direclty the parameter of the wage equation in this problem. Write the regression code that recovers them.

### **Q2** CCP given risk aversion and $r=\infty$

We start with a simple case where there is full discounting. In this case, 




The risk aversion coefficient does not enter linearly in the utility problem. We are going to fix it, and use Hotz and Miller conditional on that value. So Assume that $\rho$ is known. 
